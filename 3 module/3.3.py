...
def read_vacancy_file(file_name):
    with open(file_name, encoding="utf_8_sig") as f:
        csv = reader(f)
        headers = next(csv)
        vacancies = [v for v in csv if len([x for x in v if x]) >= len(headers) / 2]

    return headers, vacancies

def get_dictionaries(headers, data):
    dicts = []
    for vacancy in data:
        dict = {headers[i]: clean_html(value) for i, value in enumerate(vacancy)}
        dicts.append(dict)
...
def get_analysis(vacancies_dicts):
    ...
    for i, (name, employer, salary, city) in enumerate(highest_salaries):
        print(f"    {i + 1}) {name} в компании \"{employer}\" - {salary} {format_count(salary)} (г. {city})")
    print()
    ...
    for i, (city, (avg_salary, count)) in enumerate(highest_city_salaries):
        print(f"    {i + 1}) {city} - средняя зарплата {avg_salary} {format_count(avg_salary)} ({count} {get_vacancy_word(count)})")

headers, vacancies = read_vacancy_file(input())
vacancies_dicts = get_dictionaries(headers, vacancies)
